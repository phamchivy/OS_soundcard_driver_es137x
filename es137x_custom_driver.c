#define CHIP1371  /*Định nghĩa CHIP1371 để xác định loại chip đang được hỗ trợ trong module.*/
#include <linux/io.h> /*Giao tiếp với phần cứng (io.h, pci.h).*/
#include <linux/delay.h> /*Xử lý âm thanh (ALSA framework: sound/core.h, sound/pcm.h).*/
...
#include <sound/core.h> /*Quản lý điều khiển âm thanh, MIDI, hoặc codec liên quan (sound/control.h, sound/rawmidi.h).*/
...
#ifndef CHIP1371 /*Nếu không sử dụng ES1371, ES1370 sẽ được định nghĩa.*/
#undef CHIP1370
#define CHIP1370
#endif

#ifdef CHIP1370
#define DRIVER_NAME "ENS1370" /*Đặt tên driver (DRIVER_NAME) và loại chip (CHIP_NAME) phù hợp với sound card.*/
...
MODULE_AUTHOR("Pham Chi Vy, Hoang Thu Giang");
MODULE_LICENSE("GPL");
#ifdef CHIP1370
...
static int index[SNDRV_CARDS] = SNDRV_DEFAULT_IDX;	/* Index 0-MAX */     /*Các biến toàn cục được sử dụng để cấu hình sound card*/
...
MODULE_PARM_DESC(index, "Index value for Ensoniq AudioPCI soundcard."); /*cấu hình các tham số (parameters) cho kernel module*/
module_param_array(id, charp, NULL, 0444);
MODULE_PARM_DESC(id, "ID string for Ensoniq AudioPCI soundcard.");
module_param_array(enable, bool, NULL, 0444);
MODULE_PARM_DESC(enable, "Enable Ensoniq AudioPCI soundcard.");

#ifdef CHIP1371 /*spdif, lineio: Tùy chỉnh các chức năng âm thanh cụ thể của chip ES1371.*/
...
#define ES1371REV_ES1373_B  0x06	/*mã phiên bản (revision) của các chip âm thanh liên quan đến soundcard*/
 ...
 * Direct registers
 */
/*Đoạn mã này định nghĩa các macro và hằng số cho các thanh ghi (registers) của chip ES1370, ES1371 và ES1373, 
được sử dụng trong các thiết bị âm thanh như sound card.*/
...
/*
định nghĩa các thuộc tính và dữ liệu liên quan đến một thiết bị âm thanh sử dụng chip ES1370 hoặc ES1371
 */
...
static irqreturn_t snd_audiopci_interrupt(int irq, void *dev_id); /*Đây là một hàm xử lý ngắt (interrupt handler) dành cho thiết bị âm thanh AudioPCI.*/

static const struct pci_device_id snd_audiopci_ids[] = { /*Đây là một danh sách các thiết bị PCI mà trình điều khiển (driver) hỗ trợ.*/
#ifdef CHIP1370
...
MODULE_DEVICE_TABLE(pci, snd_audiopci_ids); /*driver này hỗ trợ danh sách các thiết bị PCI được định nghĩa trong snd_audiopci_ids*/

/*
 *  constants
 */

#define POLL_COUNT	0xa000 /*đặt ngưỡng hoặc kiểm soát các vòng lặp liên quan đến việc kiểm tra trạng thái phần cứng*/

#ifdef CHIP1370
static const unsigned int snd_es1370_fixed_rates[] =
	{5512, 11025, 22050, 44100};
static const struct snd_pcm_hw_constraint_list snd_es1370_hw_constraints_rates = { /*Một cấu trúc giới hạn phần cứng, chỉ ra rằng ES1370 chỉ hỗ trợ 4 tần số trong mảng*/
	.count = 4, 
	.list = snd_es1370_fixed_rates,
	.mask = 0,
};
static const struct snd_ratnum es1370_clock = { /*Định nghĩa tỷ lệ xung clock cho ES1370*/
	.num = ES_1370_SRCLOCK,
	.den_min = 29, 
	.den_max = 353,
	.den_step = 1,
...
#else
static const struct snd_ratden es1371_dac_clock = { /*Định nghĩa clock DAC (Digital-to-Analog Converter) cho ES1371*/
	.num_min = 3000 * (1 << 15),
	.num_max = 48000 * (1 << 15),
	.num_step = 3000,
	.den = 1 << 15,
...
static const struct snd_ratnum es1371_adc_clock = { /*Định nghĩa clock ADC (Analog-to-Digital Converter) cho ES1371*/
	.num = 48000 << 15,
	.den_min = 32768, 
	.den_max = 393216,
	.den_step = 1,
...
#endif
static const unsigned int snd_ensoniq_sample_shift[] = /*Đây là mảng định nghĩa các giá trị "shift" mẫu (sample shift) cho các luồng dữ liệu khác nhau.*/
	{0, 1, 1, 2};
...
#ifdef CHIP1371
/*chức năng đọc và ghi các giá trị cấu hình của bộ chuyển đổi lấy mẫu (SRC - Sample Rate Converter)*/
static unsigned int snd_es1371_wait_src_ready(struct ensoniq * ensoniq) /*Chờ SRC sẵn sàng để đọc/ghi.*/
{
	unsigned int t, r = 0;
...
static unsigned int snd_es1371_src_read(struct ensoniq * ensoniq, unsigned short reg) /*Đọc giá trị từ một thanh ghi SRC.*/
{
...
static void snd_es1371_src_write(struct ensoniq * ensoniq,
				 unsigned short reg, unsigned short data) /*Ghi giá trị vào một thanh ghi SRC.*/
{
	unsigned int r;
...
#ifdef CHIP1370
/*ghi giá trị vào thanh ghi codec ES1370 một cách an toàn*/
...
static inline bool is_ev1938(struct ensoniq *ensoniq) /*Hàm này kiểm tra xem thiết bị PCI hiện tại có phải là EV1938 (một loại chipset codec) hay không, dựa trên device ID.*/
{
	return ensoniq->pci->device == 0x8938;
}

static void snd_es1371_codec_write(struct snd_ac97 *ac97,
				   unsigned short reg, unsigned short val) /*Hàm này ghi một giá trị (val) vào một thanh ghi (reg) của codec AC'97.*/
{
...
static unsigned short snd_es1371_codec_read(struct snd_ac97 *ac97,
					    unsigned short reg) /*Hàm này cấu hình tốc độ lấy mẫu cho ADC.*/
{
	struct ensoniq *ensoniq = ac97->private_data;
...
static void snd_es1371_codec_wait(struct snd_ac97 *ac97) /*Hàm này thêm thời gian chờ để đảm bảo codec được reset và các thông tin như AC97_VENDOR_ID1, AC97_VENDOR_ID2 được đọc trước khi tiếp tục các tác vụ khác.*/
{
	msleep(750);
...
static void snd_es1371_dac2_rate(struct ensoniq * ensoniq, unsigned int rate)
{
	...
	outl(r, ES_REG(ensoniq, 1371_SMPRATE));
	mutex_unlock(&ensoniq->src_mutex);
} /*Tương tự như cấu hình ADC, các hàm snd_es1371_dac1_rate và snd_es1371_dac2_rate cấu hình tốc độ lấy mẫu cho hai kênh DAC*/

#endif /* CHIP1371 */

static int snd_ensoniq_trigger(struct snd_pcm_substream *substream, int cmd) /*xử lý các lệnh "trigger" (kích hoạt) liên quan đến việc phát và thu âm thanh trên các substream của hệ thống*/
{
 ...
 *  PCM part
 */
/*Đoạn mã này chuẩn bị 3 substream khác nhau cho các tác vụ phát lại âm thanh (playback1, playback2) và ghi âm (capture)*/
...
/*Các hàm này giúp theo dõi vị trí con trỏ (pointer) của dữ liệu trong bộ đệm DMA cho mỗi substream (playback1, playback2, capture),
đảm bảo rằng dữ liệu được đọc hoặc ghi chính xác trong quá trình phát lại hoặc ghi âm.*/
...
/*định nghĩa ba cấu trúc snd_pcm_hardware cho các substream phát lại và ghi âm trên một card âm thanh (cụ thể là Ensoniq).
Các cấu trúc này mô tả các đặc tính phần cứng và các thuộc tính mà driver sẽ sử dụng để tương tác với phần cứng âm thanh,
bao gồm các thuộc tính về định dạng âm thanh, tỷ lệ mẫu, số kênh, kích thước bộ đệm, và các thông số khác.*/
...
/*định nghĩa ba hàm mở các substream phát lại và ghi âm cho một card âm thanh Ensoniq trong một driver ALSA (Advanced Linux Sound Architecture).
Mỗi hàm này mở một substream (dòng phát lại hoặc ghi âm) và cấu hình các tham số phần cứng cũng như phần mềm cần thiết cho hoạt động của card âm thanh.*/
...
/*Các hàm trong đoạn mã bạn cung cấp là các hàm close cho các substream phát lại và ghi âm trong driver âm thanh Ensoniq.
Những hàm này được sử dụng để đóng các substream khi không còn cần thiết, đồng thời giải phóng các tài nguyên và cập nhật các chế độ tương ứng.*/
...
/*Đoạn mã này định nghĩa hai hàm snd_ensoniq_pcm và snd_ensoniq_pcm2, được sử dụng để thiết lập và khởi tạo các đối tượng PCM (Pulse Code Modulation)
trong trình điều khiển (driver) cho card âm thanh Ensoniq. Các hàm này thiết lập các luồng phát và ghi âm thanh, cũng như cấu hình các bộ đệm và ánh xạ kênh âm thanh.*/
...
static int snd_ensoniq_pcm_custom(struct ensoniq *ensoniq, int device)
{
    struct snd_pcm *pcm;
    int err;

    /* Tạo PCM mới, thay đổi tham số cho tên PCM và loại device */
    err = snd_pcm_new(ensoniq->card, CHIP_NAME "/custom", device, 2, 1, &pcm); // 2 substreams, 1 capture
    if (err < 0)
        return err;

    /* Thiết lập các operations cho từng substream */
    snd_pcm_set_ops(pcm, SNDRV_PCM_STREAM_PLAYBACK, &snd_ensoniq_playback1_ops); // Playback substream 1
    snd_pcm_set_ops(pcm, SNDRV_PCM_STREAM_PLAYBACK, &snd_ensoniq_playback2_ops); // Playback substream 2
    snd_pcm_set_ops(pcm, SNDRV_PCM_STREAM_CAPTURE, &snd_ensoniq_capture_ops); // Capture substream

    pcm->private_data = ensoniq;
    pcm->info_flags = 0;
    
    /* Thêm tên cho PCM */
    strcpy(pcm->name, CHIP_NAME " Custom DAC");
    ensoniq->pcm_custom = pcm;

    /* Cấu hình bộ đệm cho PCM */
    snd_pcm_set_managed_buffer_all(pcm, SNDRV_DMA_TYPE_DEV,
                                   &ensoniq->pci->dev, 64*1024, 128*1024);

    /* Thêm các điều khiển âm thanh cho substreams (ví dụ: surround map, standard channels) */
#ifdef CHIP1370
    err = snd_pcm_add_chmap_ctls(pcm, SNDRV_PCM_STREAM_PLAYBACK,
                                 snd_pcm_std_chmaps, 2, 0, NULL); // Standard channel map
#else
    err = snd_pcm_add_chmap_ctls(pcm, SNDRV_PCM_STREAM_PLAYBACK,
                                 surround_map, 2, 0, NULL); // Surround map (2 channels)
#endif

    /* Đảm bảo đã cấu hình và trả về lỗi nếu có */
    return err;
}
				...
				const struct pci_device_id *pci_id)
{
	...
	if (err < 0)
		return err;
	err = snd_ensoniq_pcm_custom(ensoniq, 2);
	if (err < 0)
		return err;
